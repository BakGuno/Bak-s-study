...
namespace Week5
{
    internal class Flood_Fill2
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("시작 지점을 입력해주세요. ", Col, row);
            string start = Console.ReadLine();
            string[] starts = start.Split(",");
            int startx = int.Parse(starts[0]); ;
            int starty = int.Parse(starts[1]);
            visited[starty][startx] = true; //시작지점만 true로
            ...
            Find(arr, startx, starty, visited, change);
        ...
        static int[][] Find(int[][] arr, int x, int y, bool[][] visited, int change) //조건이 다채로움.
        {
            if (y >= 1 && x < arr[y - 1].Length)
                if (arr[y - 1].Length >= x)
                    if (arr[y - 1][x] == arr[y][x])
                        if (visited[y - 1][x] == false)
                        {
                            visited[y - 1][x] = true;
                            Find(arr, x, y - 1, visited, change);
                        }


            if (y < arr.Length - 1 && x < arr[y+1].Length)
                if (arr[y + 1][x] == arr[y][x])
                    if (visited[y + 1][x] == false)
                    {
                        visited[y + 1][x] = true;
            ...
            arr[y][x] = change; //들리는게 끝나면 맨 끝 방문한 곳부터 차례차례 숫자를 바꾸면서 나옴.  
            return arr; //바뀐 배열 반환
        }
    }
}
